          ...
          if (schema === "entity") {
            ...
            resolve(
              rows.map((row) => ({         
                ...
                text_unit_ids: ensureArray(row["text_unit_ids"]),
            ...
            resolve(
              rows.map((row) => ({
                ...
                document_ids: ensureArray(row["document_ids"]),
                entity_ids: ensureArray(row["entity_ids"]),
                relationship_ids: ensureArray(row["relationship_ids"]),
              }))
            );
            ...
            resolve(
              rows.map((row) => ({
                ...
                findings: (() => { try { return typeof row["findings"] === "string" && row["findings"] ? JSON.parse(row["findings"]) : (row["findings"] || []); } catch { return []; } })(),
                full_content_json: (() => { try { return typeof row["full_content_json"] === "string" && row["full_content_json"] ? JSON.parse(row["full_content_json"]) : (row["full_content_json"] || ""); } catch { return ""; } })(),
                period: row["period"],                
                size: parseValue(row["size"], "number"),            
              }))
            );
