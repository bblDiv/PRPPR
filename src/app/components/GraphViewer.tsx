...
const GraphViewer: React.FC<GraphViewerProps> = ({
  ...
  return (
    <Box
      ref={containerRef}
      sx={{
        height: embedded ? "100%" : isFullscreen ? "100vh" : "calc(100vh - 64px)",
        width: embedded ? "100%" : isFullscreen ? "100vw" : "100%",
        position: embedded ? "absolute" : isFullscreen ? "fixed" : "relative",
        top: 0,
        left: 0,
    ...
    >
      ...
      {graphType === "2d" ? (
        ...
        <ForceGraph3D
          ...
          nodeAutoColorBy="type"
          nodeRelSize={NODE_R}
          {...(embedded ? { width: dimensions.width, height: dimensions.height } : {})}
          linkWidth={(link) =>
            showHighlight && highlightLinks.has(link) ? 5 : 1
          }
          linkDirectionalParticles={showHighlight ? 4 : 0}
